...
do
  ...
  aprun -L $bbos_server -n 2 -N 32 -d 2 $bbos_server_path -c $config_dir/$bbos_server_config_name.$bbos_server
  ...
  do
    ...
    if [[ $n -lt $num_bbos_clients_per_server ]]; then
      aprun -L $bbos_client -n 1 -N 1 -d 1 $bbos_client_path -c $config_dir/$bbos_client_config_name.$bbos_server
      n=$(($n + 1))
